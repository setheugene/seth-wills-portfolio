...
class RouteProcessor
{
    ...
    public static function init(Router $router, array $routes = array())
    {
        ...
        add_action('template_redirect', array($self, 'caRoute_hook'));
    }

    /**
     * Checks to see if a route was found. If there's one, it calls the route hook.
     */
    public function caRoute_hook()
    {
        if ( is_admin() )
            return;
        ...
        if ( !$this->matched_route->is_rest() && $this->matched_route->get_middleware() !== 'open' ) {
            return causer_func_array( 'Middleware::'.$this->matched_route->get_middleware(), array($this->router) );
        }
    ...
    {
        $routes = apply_filters('Router_routes', $this->routes);
        ...
        if ($routes_hash != get_option('Router_routes_hash')) {
            flush_rewrite_rules();
            update_option('Router_routes_hash', $routes_hash);
        }
    }
